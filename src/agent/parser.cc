...
SpParser::GetRuntimeSymtabs(sp::SymtabSet& symtabs) {
  ...
  if (!al) {
    sp_debug_agent("FAILED TO CREATE AddressLookup");
    return NULL;
  }
  al->refresh();
  ...
  if (tabs.size() == 0) {
    sp_debug_agent("FOUND NO SYMTABS");
    return NULL;
  }
  sp_debug_agent("SYMTABS - %ld symtabs found", (long)tabs.size());
  ...
  for (std::vector<sb::Symtab*>::iterator i = tabs.begin();
    ...
    if (sym->findSymbol(syms, magic_var) && syms.size() > 0) {
      ...
      sp_debug_agent("AGENT NAME - %s", agent_name_.c_str());
    }
...
SpParser::CreatePatchobjs(sp::SymtabSet& unique_tabs,
  ...
  for (SymtabSet::iterator i = unique_tabs.begin();
    ...
    if (!CanInstrumentLib(libname_no_path)
      ...
      sp_debug_agent("SKIPED - skip parsing %s",
    ...
    if (!patch_obj) {
      sp_debug_agent("FAILED TO CREATE PATCH OBJECT");
      continue;
    }

    patch_objs.push_back(patch_obj);
    sp_debug_agent("PARSED - parsed %s", sp_filename(sym->name().c_str()));

    if (sym->isExec()) {
      exe_obj_ = patch_obj;
      sp_debug_agent("EXE - %s is an executable",
               sp_filename(sym->name().c_str()));
    }
...
SpParser::GetExeFromProcfs(sp::PatchObjects& patch_objs) {
  ...
  for (PatchObjects::iterator oi = patch_objs.begin();
    ...
    sp_debug_agent("Trying next patch obj");
    ...
    sp_debug_agent("exec name[%s] obj name[%s]", sp::GetExeName().c_str(), obj->name().c_str());
    if (strcmp(s1, s2) == 0) {
      sp_debug_agent("GOT EXE - %s is an executable shared library", s2);
      return obj;
    } // strcmp
    else {
      sp_debug_agent("PARSE EXE - s1=%s, s2 =%s", s1, s2);
    }
  } // for each obj
  sp_debug_agent("failed to find exe from procfs");
  return NULL;
}
...
SpParser::CreateObject(sb::Symtab* sym,
  ...
  if (getenv("SP_PARSE_DIR")) {
    ...
    if (dir) {
      ...
      while (entry) {
        sp_debug_agent("Parsed library: %s is %s?", entry->d_name,
                 no_path_name);
        if (strcmp(no_path_name, entry->d_name) == 0) {
          sp_debug_agent("GOT Parsed LIB: %s", entry->d_name);
      ...
      sp_debug_agent("FAILED TO ITERATE DIR %s", getenv("SP_PARSE_DIR"));
    }
  }

  if (parse_from_file) {
    sp_debug_agent("PARSE FROM FILE - for %s", sym->name().c_str());
    obj = CreateObjectFromFile(sym, load_addr);
  } else {
    sp_debug_agent("PARSE FROM RUNTIME - for %s", sym->name().c_str());
...
SpParser::CreateObjectFromRuntime(sb::Symtab* sym,
  ...
  sp_debug_agent("Start create obj from runtime symtab name: %s", sym->name().c_str());
...
// dt::Address real_load_addr =
  ...
  dt::Address real_load_addr=load_addr;	
   sp_debug_agent("Symbol %s: load_addr: %lx, scs->loadAddress: %lx", sym->name().c_str(),load_addr,
           scs->loadAddress());
...
SpParser::Parse() {
  ...
  sp_debug_agent("Created patchapi objs");
 ...
 public:
  ...
  virtual void visit(in::RegisterAST* r) {
    if (r->getID().isPC()) {
      ...
      // sp_debug_agent("GOT NON-PC REG - %s = %lx",
      //          r->getID().name().c_str(), rval);
      call_addr_ = rval;
    }

    // sp_debug_agent("SP_VISITOR - reg value %lx", call_addr_);
  ...
  virtual void visit(in::BinaryFunction* b) {
    ...
    if (b->isAdd()) {
      call_addr_ = i1 + i2;
      // sp_debug_agent("SP_VISITOR - %lx + %lx = %lx", i1, i2, call_addr_);
  ...
  virtual void visit(in::Immediate* i) {
    ...
    // sp_debug_agent("SP_VISITOR - imm %lx ", call_addr_);
    stack_.push(call_addr_);
  }
  virtual void visit(in::Dereference* d) {
    dt::Address* addr = (dt::Address*)(stack_.top() + seg_val_);
    stack_.pop();
    // sp_debug_agent("SP_VISITOR - dereferencing %lx => ? ",
    //          (dt::Address)addr);
    call_addr_ = *addr;
    // sp_debug_agent("SP_VISITOR - dereference %lx => %lx ",
...
SpParser::callee(SpPoint* pt,
  ...
  if (f) {
    ...
    //   f = tmp_f;
    // }
    sp_debug_agent("got callee, found function %lx", (long unsigned int) f);
  ...
  if (parse_indirect) {
    ...
    if (addr_callee_not_found_.find(b->last()) !=
      ...
      sp_debug_agent("NOT FOUND - proved not found for %lx", b->last());
      return NULL;
    }
...
SpParser::GetFuncAddrFromName(string name) {
  ...
  if (!FindFunction(name, &found_funcs) || found_funcs.size() == 0) {
    sp_debug_agent("GET FUNC ADDR FROM NAME - failed for %s", name.c_str());
    return 0;
  }

  // To simplify things, We don't want the payload function to be overloaded
  if (found_funcs.size() > 1) {
    sp_debug_agent("GET FUNC ADDR FROM NAME - failed for %s, overloaded func",
             name.c_str());
    return 0;
  }
...
SpParser::FindFunction(dt::Address addr) {
  sp_debug_agent("FIND FUNC BY ADDR - for call insn %lx", addr);
  ...
  if (addr_func_not_found_.find(addr) != addr_func_not_found_.end()) {
    sp_debug_agent("NOT FOUND - call at %lx is proved to be not found", addr);
    return NULL;
  }
  ...
  for (ph::AddrSpace::ObjMap::iterator ci = as->objMap().begin();
    ...
    if (!CanInstrumentLib(obj->name().c_str())) {
      sp_debug_agent("SKIPED - skip finding function at %s",
    ...
    sp_debug_agent("code base %lx for %s", obj->codeBase(), obj->name().c_str());
    ...
    if(cnt != 1) {
      sp_debug_agent("%ld Regions found", cnt);
      continue;
    }
    else if(cnt == 1) {
      co->findBlocks(*match.begin(), offset, blocks);

      sp_debug_agent("%ld blocks found", (long)blocks.size());
      if (blocks.size() == 1) {
  ...
  sp_debug_agent("Function not found");
  addr_func_not_found_.insert(addr);
  return NULL;
}
...
SpParser::FindFunction(string name,
                       FuncSet* found_funcs) {
  sp_debug_agent("LOOKING FOR FUNC BY PRETTY NAME - looking for %s", name.c_str());
  ...
  if (demangled_func_map_.find(name) != demangled_func_map_.end()) {
    sp_debug_agent("GOT FROM CACHE - %s", name.c_str());
  ...
  for (ph::AddrSpace::ObjMap::iterator ci = as->objMap().begin();
    ...
    if (!CanInstrumentLib(sp_filename(obj->name().c_str()))) {
      sp_debug_agent("Find Function: SKIP - lib %s for the function %s", sp_filename(obj->name().c_str()),name.c_str());
      continue;
    }
...
SpParser::FindFunctionByMangledName(string name) {
  ...
  for (ph::AddrSpace::ObjMap::iterator ci = as->objMap().begin();
    ...
    // 2. In inst_lib, lib is libc, but function is not __libc_start_main
    if (!CanInstrumentLib(sp_filename(obj->name().c_str()))) {
      sp_debug_agent("SKIP - lib %s for the function %s", sp_filename(obj->name().c_str()),name.c_str());
      continue;
    }
...
SpParser::FindFunction(string name) {
  ...
  if (func_set.size() == 1) {
    ...
    sp_debug_agent("FOUND - %s in object %s", name.c_str(),
             FUNC_CAST((*func_set.begin()))->GetObject()->name().c_str());
...
SpParser::GetFuncsByName(sp::SpObject* obj,
  ...
  assert(obj);
  assert(func_set);
  sp_debug_agent("IN OBJECT - %s in %s?",
  ...
  if ((strcmp(name.c_str(), "recv") != 0) && (strcmp(name.c_str(), "read") != 0) && obj->name().find("libc-") != std::string::npos &&
    ...
    sp_debug_agent("Find function: SKIP - lib %s and non __libc_start_main for the function %s",
  ...
  for (pe::CodeObject::funclist::iterator fit = all.begin();
    ...
    if (mangled) {
      if (!found ||
        ...
        /*
          sp_debug_agent("SKIP %s - mangled name %s length %ld",
          (*fit)->name().c_str(),
      ...
      if (!found ||
        ...
        // sp_debug_agent("found [%s] name [%s]", found->name().c_str(), name.c_str());
        continue;
      }
    }
        
    sp_debug_agent("GOT %s in OBJECT - %s at %lx",
...
SpParser::FindPltFunc(sp::SpObject* obj,
  ...
  if (!symtab->getFuncBindingTable(fbt)) {
      sp_debug_agent("No relocation information for this Symtab");
      return false;
  }
...
*/
  ...
  for (u_int i = 0; i < fbt.size(); i++) {
		 ...
		 sp_debug_agent("Plt function address %lx", pltFuncAddr);
                // check to see if this function has been bound yet
                if (hasBeenBound(fbt[i],objLoadAddr,func_set)) {
		   sp_debug_agent("GOT %s from FindPltFunc, Object - %s",
        ...
        sp::FuncSet* func_set)
{
   ...
   // dt::Address bound_addr = 0;
    sp_debug_agent("function relative address %lx, object base address %lx, got_entry %lx",entry.rel_addr(),base_addr,got_entry);
    //unsigned tmp = 0;

    //Read the value in the address got_entry
    sp_debug_agent("Bound address %lx ", got_entry);//bound_addr);
        // the callee function has been bound by the runtime linker
        // find function and return
	SpFunction* func=FindFunction(got_entry);
        if (func==NULL) {
            sp_debug_agent("FindFunction in hasbeenBound failed");
            return false;
        } else {
		sp_debug_agent("Function from FindFunction(Address) %s",func->name().c_str());
