...
SpAddrSpace::DumpMemoryMappings() {
  ...
  sp_debug_patchapi("MMAPS - %lu memory mappings",
  ...
  for (MemMappings::iterator mi = mem_maps_.begin();
    ...
    sp_debug_patchapi("MMAP - Range[%lx ~ %lx], Offset %lx, Perm %x, Dev %s,"
...
SpAddrSpace::UpdateFreeIntervals() {
  ...
  for (ph::AddrSpace::ObjMap::iterator i = obj_map_.begin();
    ...
    assert(obj);
    sp_debug_patchapi("HANDLING OBJECT - %s @ load addr: %lx, code base: %lx",
    ...
    if (!GetClosestInterval(mapping.start, &interval)) {
      sp_debug_patchapi("FAILED TO GET FREE INTERVAL - for %lx %s",
    ...
    sp_debug_patchapi("GET FREE INTERVAL - [%lx, %lx], w/ original size %ld, "
...
SpAddrSpace::DumpFreeIntervals() {
  for (FreeIntervalList::iterator i = free_intervals_.begin();
    ...
    sp_debug_patchapi("FREE INTERVAL - [%lx ~ %lx], used %d, size: %ld MB",
...
SpAddrSpace::allocateNewInterval(SpObject* obj) {
  ...
  sp_debug_patchapi("Trying to allocate new interval for object %s", obj->name().c_str());
...
SpAddrSpace::GetClosestInterval(dt::Address addr,
  ...
  for (FreeIntervalList::iterator i = free_intervals_.begin();
    ...
    sp_debug_patchapi("GET CLOSEST INTERVAL - free interval [%lx, %lx], size %ld",
             (*i).start, (*i).end, (*i).size());
    if ((*i).start > addr) {
      sp_debug_patchapi("start larger than addr");
      continue;
    } else {
      if (!previous) return false;
      if (previous->used) {
        sp_debug_patchapi("this interval already used");
        continue;
      }
      if ((addr - (*i).start) > distance) {
        sp_debug_patchapi("distance too far");
        continue;
      }
      *interval = previous;
      (*interval)->used = true;
