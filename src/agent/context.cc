  ...
  SpContext::GetCallStack(FuncSet* call_stack) {
    ...
    */
    ...
    for (unsigned i=0; i<stackwalk_.size(); i++) {
      ...
      if (!func) {
        sp_debug("agent", "SKIPPED - Function %s cannot be resolved", s.c_str());
        continue;
      }

      // Step 2: add this function
      sp_debug("agent", "FOUND - Function %s is in the call stack", s.c_str());
  ...
  dt::Address SpContext::GetReturnAddress()
  {
	for(unsigned i=0;i <stackwalk_.size() ;i++) {
		...
		if(IsRecvLikeFunction(func.c_str())) {
			...
			sp_debug("agent", "Function %s Return Address on the stack  = %lx",func.c_str(),location.val.addr);
