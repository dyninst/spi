...
namespace sp {
  ...
  RelocCallInsnWorker::run(SpPoint* pt) {
    sp_debug_worker("RELOC CALLINSN WORKER - runs");
    ...
    char* call_insn = (char*)call_insn_addr;
		...
		if (call_insn_size < 5) {
			sp_debug_worker("SMALL CALL INSN - call insn is too small (< 5-byte),"
							 " try other workers");
			return false;
		}
    ...
    if (call_insn[0] != (char)0xe8) {  // A direct call?
			sp_debug_worker("NOT NORMAL CALL");
			if (!pt->tailcall()) {           // A jump for direct tail call?
				sp_debug_worker("NOT TAIL CALL - try other workers");
				return false;
			} else {
				sp_debug_worker("IS TAIL CALL, call insn size: %lu", call_insn_size);
        sp_debug_worker("Try other workers");
        return false;
    ...
    if (!sp::IsDisp32(rel_addr)) {
      sp_debug_worker("NOT 4-byte DISP - blob=%lx, call_insn=%lx, delta=%ld,"
  ...
  RelocCallInsnWorker::install(SpPoint* pt) {
    ...
    *p++ = 0xe9;
    ...
    int* lp = (int*)p;
		...
		assert(pt->callee());
    sp_debug_worker("BEFORE INSTALL (%lu bytes) for point %lx for %s- {",
    ...
    *lp = (int)((long)blob - (long)call_addr - insn_length);
    ...
    sp_debug_worker("REL CAL - blob at %lx, last insn at %lx, insn length %lu",
             (unsigned long)blob, b->end(),
						 (unsigned long)insn_length);
    sp_debug_worker("JUMP REL - jump to relative address %x", *lp);
    ...
    sp_debug_worker("AFTER INSTALL (%lu bytes) for point %lx for %s- {",
             b->size(), b->last(),
             pt->callee()->name().c_str());
    sp_debug_worker("%s", g_parser->DumpInsns((void*)b->start(),
																			 b->size()).c_str());
