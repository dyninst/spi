  ...
  TEST_F(IntegrationTest, gcc) {
		...
		if (getenv("PLATFORM")) {
			if (strcmp(getenv("PLATFORM"), "x86_64-unknown-linux2.4") == 0) {
				gcc_path = "LD_LIBRARY_PATH=/scratch/wenbin/software/lib:$LD_LIBRARY_PATH /scratch/wenbin/software/libexec/gcc/x86_64-unknown-linux-gnu/4.5.2/cc1";
			} else {
				gcc_path = "/afs/cs.wisc.edu/s/gcc-4.6.1/@sys/libexec/gcc/i686-pc-linux-gnu/4.6.1/cc1";
			} 
		} else {
			sp_perror("PLATFORM environment variable undefined");
		}
