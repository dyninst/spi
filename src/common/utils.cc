...
SetupTimer(const int i) {
  if (i < 0 || i > (kNumTimers - 1)) {
    sp_debug_common("OUT OF BOUND TIMER - timer id should be in [0, 9]");
    return;
  }
  a[i]._clocks = 0;
  a[i]._start = 0;
  ...
  while ((de = readdir(dir)) != 0) {
    if (isdigit(de->d_name[0])) {
      ...
      if (pid != getpid() &&
        ...
        sp_debug_common("GET PID FOR PIPE: pid - %d", pid);
        pid_set.insert(pid);
      }
    }
  }
...
void GetSocketDescFromPid(int pid, SocketSet& socket_set)
{
  ...
  while ((de = readdir(dir)) != 0) {
    if (isdigit(de->d_name[0])) {
      ...
      int size = -1;
      if ((size = readlink(name, buffer, kLenStringBuffer)) < 0) {
        sp_debug_common("GetSocketDescFromPid: readlink error");
      }
      else {
        buffer[size] = '\0';
        ...
        sp_debug_common("%d uses Socket %d",pid,dir);
       }
     }
    }
  }   // Iterate all processes
...
GetLocalAddress(const int fd,
  ...
  if (getsockname(fd, reinterpret_cast<sockaddr*>(out), &sock_len) == -1) {
    sp_debug_common("FAILED GET LOCAL ADDR - getsockname @ pid = %d", getpid());
    return false;
  }
  return true;
}
...
GetRemoteAddress(const int fd,
  ...
  if (getpeername(fd, reinterpret_cast<sockaddr*>(out), &sock_len) == -1) {
    sp_debug_common("FAILED GET REMOTE ADDR - getpeername @ pid = %d", getpid());
    return false;
  }
  return true;
}
...
/*  std::string proc_path = "";
  ...
  for (StringSet::iterator si = illegal_exes.begin();
    ...
    // sp_debug_common("comparing: %s", (*si).c_str());
...
GetExeName() {
  ...
  sp_debug_common("Exename is %s",exe);
...
ProcessHasLibrary(int pid, std::string lib)
{
  ...
  while(std::getline(fp,line))
  {
      ...
      {
        sp_debug_common("Agent shared library is already injected into the process");
        return true;
      }
 }
 return false;
...
void sig_urg_handler(int signo)
{
        ...
        char    buff[100];
 	int connfd;
	sp_debug_common("SIGURG received!!");
        ...
        for(SocketSet::iterator s=socket_set.begin(); s!=socket_set.end(); s++) {
   	   connfd=*s;
	   sp_debug_common("Socket: %d, trying to receive OOB", connfd);
...
InitLock(SpLock* const lock) {
  assert(lock);
  sp_debug_common("INIT LOCK - mutex %d", lock->mutex);
  lock->mutex = 0;
  return 0;
}
...
Unlock(SpLock* const lock) {
  assert(lock);
  // sp_debug_common("UNLOCK - mutex %d", lock->mutex);
  __sync_synchronize();
  lock->mutex = 0;
  return 0;
}
...
int shmid;
  ...
  int ret = shmctl(shmid, IPC_RMID, NULL);
    if (ret < 0) {
      sp_debug_common("free shared memory failed");
    } else {
      sp_debug_common("free shared memory successful");
    }
  return;
}
