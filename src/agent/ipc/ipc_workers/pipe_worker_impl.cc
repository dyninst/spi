...
SpPipeWorker::SpPipeWorker() {
  // sp_debug_ipc("PIPE WORKER - created for pid=%d", getpid());
...
SpPipeWorker::Inject(SpChannel* c,
  ...
  sp_debug_ipc("NO INJECTED(pipe) -- start injection");

  if (c->remote_pid > 0) {

    sp_debug_ipc("Remote pid is %d",c->remote_pid);
    if(ProcessHasLibrary(c->remote_pid, "libagent")){
     sp_debug_ipc("Process %d already has agent shared library", c->remote_pid);
     return true;
    }
    SpInjector::ptr injector = SpInjector::Create(c->remote_pid);
    if(!injector)
	sp_debug_ipc("Injector not created");
    else
    sp_debug_ipc("Injector created for process id %d",c->remote_pid);
    string agent_name = "";
    ...
    sp_debug_ipc("agent shared library which is going to be injected is %s",agent_name.c_str());
    c->injected = injector->Inject(agent_name.c_str());
    sp_debug_ipc("Returned back");
  } 
   if(c->injected)
	sp_debug_ipc("Injected into the pipe");
   else
	sp_debug_ipc("Not injected into the pipe");
  return c->injected;
//  return 0;
}
...
SpPipeWorker::CreateChannel(int fd,
  ...
  sp_debug_ipc("FD TO PID - get a %lu pids from fd %d",
           (unsigned long)pid_set.size(), fd);
  if (pid_set.size() == 0) {
    sp_debug_ipc("FAILED TO GET PID - from fd = %d", fd);
    // return NULL;
  }
  ...
  for (PidSet::iterator i = pid_set.begin(); i != pid_set.end(); i++) {
    if (*i != c->local_pid) {
      c->remote_pid = *i;
      sp_debug_ipc("Local pid =%d Remote pid = %d",c->local_pid,c->remote_pid);
      break;
    }
  }
  c->type = SP_PIPE;
