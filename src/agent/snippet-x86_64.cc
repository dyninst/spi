...
#define RBP (-64)
  ...
  /*
    for (int i = -64; i < 48; i++) {
    sp_debug_agent("DUMP SAVED REGS: %lx", reg_val((i*8)));
    }
  */
  dt::Address out = 0;
  if (GetRegInternal(reg, &out)) {
    sp_debug_agent("GOT REG - %s = %lx", reg.name().c_str(), out);
    return out;
  }
  ...
  if (reg == d64::rsp || reg == d64::esp || reg == d64::sp || reg == d64::spl) {
    sp_debug_agent("GOT SP - %s = %lx = %lx + %x", reg.name().c_str(),
             saved_context_loc_ + RSP, saved_context_loc_, RSP);
    if (func()) {
      sp_debug_agent("Before %s", func()->name().c_str());
    }
    return (saved_context_loc_ + RSP);
  }

  sp_debug_agent("NOT FOUND - %s", reg.name().c_str());
  return 0;
}
 ...
 public:
  ...
  virtual void visit(in::RegisterAST* r) {
    ...
    if (r->getID() == Dyninst::x86_64::rip) {
      ...
      imm_ = a_;
      stack_.push(imm_);
      sp_debug_agent("EMU VISITOR - pc %lx", a_);
    }
  }
  virtual void visit(in::BinaryFunction* b) {
    assert(b);
    ...
    if (b->isAdd()) {
      imm_ = i1 + i2;
      sp_debug_agent("EMU VISITOR - %lx + %lx = %lx", i1, i2, imm_);
  ...
  virtual void visit(in::Immediate* i) {
    ...
    sp_debug_agent("EMU VISITOR - IMM %lx", imm_);
    stack_.push(imm_);
  }
  virtual void visit(in::Dereference* d) {
  ...
  emulate_pcsen(in::Instruction::Ptr insn,
    ...
    *p++ = 0x49;
    ...
    if ((char)insn_buf[1] == (char)0x8d) {
      ...
      sp_debug_agent("LEA - orig-disp(%d), orig-insn-addr(%lx),"
    ...
    // Deal with non-lea instruction
      ...
      *l = visitor.imm();
      ...
      sp_debug_agent("OTHER PC-INSN - orig-insn-size(%ld), abs-trg(%lx)",
               insn->size(), *l);
...
bool SpSnippet::getTargetAddr(dt::Address a, in::Instruction insn,
  ...
  for (set<in::Expression::Ptr>::const_iterator iter = mems.begin();
    ...
    if (exp->bind(thePC.get(), in::Result(in::u64, a + insn.size())) ||
      ...
      if (!res.defined) {
        sp_debug_agent("ERROR: failed bind/eval at %lx", a);
        continue;
      }
      assert(res.defined);
...
size_t SpSnippet::reloc_insn_internal(dt::Address a, in::Instruction insn,
  ...
  if (use_pc) {
    ...
    sp_debug_agent("READ USE PC: %s",
             g_parser->DumpInsns((void*)insn.ptr(), insn.size()).c_str());
    if (!getTargetAddr(a, insn, targetAddr))
      sp_debug_agent("Cannot obtan target address");
    sp_debug_agent("Target address is %lx", targetAddr);
    ...
    *newInsn++ = *origInsn++;
    ...
    if (is_data_abs64) {
      ...
      *((int*)newInsn) = (int)(newDisp - insnSz);
      ...
      sp_debug_agent("Target address is 32 bit");
      ...
      *((int*)newInsn) = (int)(targetAddr);
      ...
      sp_debug_agent("Reaching what should not be reached");
      assert(0);
    }
...
size_t SpSnippet::reloc_insn(dt::Address src_insn, in::Instruction insn,
  ...
  if (use_pc) {
    assert(opSet.size() == 1);
    sp_debug_agent("USE PC: %s",
             g_parser->DumpInsns((void*)insn.ptr(), insn.size()).c_str());
    sp_debug_agent("USE_PC - at %lx", src_insn);
  } else {
    sp_debug_agent("NOT_USE PC - at %lx", src_insn);
  }
...
size_t SpSnippet::emit_call_orig(char* buf, size_t offset) {
  ...
  sp_debug_agent("EMIT_CALL_ORIG - for call insn at %lx", src);
  sp_debug_agent("BEFORE RELOC - %s",
           g_parser->DumpInsns((void*)insn.ptr(), insn.size()).c_str());
  if (use_pc) sp_debug_agent("PC-REL CALL");
  size_t insn_size = reloc_insn_internal(src, insn, opSet, use_pc, p);
  sp_debug_agent("AFTER RELOC %s", g_parser->DumpInsns((void*)p, insn_size).c_str());
  return insn_size;
}
...
dt::Address SpSnippet::GetFs(void* context) {
  ...
  sp_debug_agent("REG_CSGSFS=%llx", ctx->uc_mcontext.gregs[index]);
