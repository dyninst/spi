...
static char dlopen_code[] = {
  0x50,                       /* pushl %eax */
  0x68, 0x0, 0x0, 0x0, 0x0,   /* pushl Lmid_t */
  0x68, 0x0, 0x0, 0x0, 0x0,   /* pushl caller */
  0x68, 0x0, 0x0, 0x0, 0x0,   /* pushl RTLD_NOW */
  0x68, 0x0, 0x0, 0x0, 0x0,   /* pushl libname  */
  0xe8, 0x0, 0x0, 0x0, 0x0,   /* call _dl_open    */
  0x58,                       /* popl %eax */
  0x58,                       /* popl %eax */
  0x58,                       /* popl %eax */
  0x58,                       /* popl %eax */
...
class Injector {
  public:
    ...
    void inject(char* code, char* lib, size_t size) {
      if (!proc_->stopProc()) return;
      Dyninst::Address dlopen_addr = find_dlopen();
      cerr << "_dl_open is at: " << std::hex << dlopen_addr << "\n";

      Dyninst::Address do_dlopen_addr = find_do_dlopen();
      cerr << "do_dlopen is at: " << std::hex << do_dlopen_addr << "\n";

      /* push %eip value to %esp */
      ThreadPool& tp = proc_->threads();
      std::cerr << tp.size() << " threads\n";
      ...
      /*
           ...
           0xcaller
           eax
           libm.so (left->top) <-esp
           old eip
       */
      esp -= (strlen(lib) + 1);
      Dyninst::Address lib_addr = esp;
      ...
      unsigned long* p = (unsigned long*)&code[OFF_LIBNAME];
      *p = lib_addr;
      p = (unsigned long*)&code[OFF_DLOPEN];
      *p = (unsigned long)dlopen_addr - (unsigned long)(code_addr + OFF_DLRET);
      p = (unsigned long*)&code[OFF_CALLER];
      //*p = (unsigned long)(code_addr + OFF_DLRET);
      *p = (unsigned long)do_dlopen_addr;
      p = (unsigned long*)&code[OFF_FLAG];
      *p = (unsigned long)RTLD_NOW;
      p = (unsigned long*)&code[OFF_LMID];
      //*p = (unsigned long)LM_ID_BASE;
      *p = (unsigned long)LM_ID_NEWLM;
      // *p = (unsigned long)0;
...
int main(int argc, char *argv[]) {
  ...
  injector.inject(dlopen_code, "./libagent.so", sizeof(dlopen_code));
