...
namespace sp {
  ...
  SpAddrSpace::SetMemoryPermission(dt::Address a,
                                    ...
                                    int perm) {
    bool ret = false;
    sp_debug_patchapi("SETTING MEMORY PERM - for [%lx ~ %lx] of %ld bytes",
             (long)a, (long)(a + length - 1), (long)length);

    dt::Address aligned = a;
    size_t pz = getpagesize();
    if (a > pz) {
      sp_debug_patchapi("PAGE SIZE SMALLER - pagesize %lx, address %lx",
               (long)pz, (long)a);
      aligned = (dt::Address)(((dt::Address) aligned) & ~(pz-1));
    } else if (a % pz == 0) {
      aligned = a;
    } else {
      sp_debug_patchapi("PAGE SIZE LARGER - pagesize %lx, address %lx",
               (long)pz, (long)a);
    ...
    sp_debug_patchapi("TRY mprotect - for [%lx ~ %lx] of %ld bytes",
             (long)aligned, (long)(aligned + len - 1), (long)len);
    if (mprotect((void*)aligned, len, perm) < 0) {
      sp_debug_patchapi("MPROTECT - Failed to change memory access permission");
      // perror("mprotect");
      return false;
    } else {
      sp_debug_patchapi("MPROTECT - SUCCEED TO change memory access"
