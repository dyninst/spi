...
//////////////////////////////////////////////////////////////////////
SpInjector::ptr SpInjector::Create(dt::PID pid) {
...
SpInjector::SpInjector(dt::PID pid) : pid_(pid) {
  ...
  if (proc_ == Process::ptr()) {
    sp_debug_injector("Injector [pid = %5d] - Failed to attach to process %d.", getpid(),
             pid);
    exit(0);
  } else
    sp_debug_injector("Attached the process %d successfully", pid);
...
SpInjector::~SpInjector() {
  sp_debug_injector("Detaching the process");
  proc_->detach();
}

//////////////////////////////////////////////////////////////////////
// Find function in all loaded libraries
Address SpInjector::FindFunction(const char *func) {
  ...
  for (LibraryPool::iterator li = libs.begin(); li != libs.end(); li++) {
    ...
    if (name.size() <= 0)
      continue;
    sp_debug_injector("IN LIB - %s", name.c_str());
    sb::Symtab *obj = NULL;
    bool ret = sb::Symtab::openFile(obj, name);
    std::vector<sb::Function *> funcs;
    if (!obj || !ret) {
      sp_perror("Injector [pid = %5d] - Failed to open %s", getpid(),
                sp_filename(name.c_str()));
    } else {
...
Address SpInjector::FindDlopen() {
  Address func_addr = FindFunction("do_dlopen");
  if (func_addr)
    return func_addr;

  sp_debug_injector("Injector [pid = %5d]: Cannot find do_dlopen", getpid());

  func_addr = FindFunction("__libc_dlopen_mode");
  sp_debug_injector("Injector [pid = %5d]: Got _dl_open at %lx", getpid(), func_addr);
...
Process::cb_ret_t on_event_lib(Event::const_ptr ev) {
  ...
  for (std::set<Library::ptr>::iterator i = libs.begin(); i != libs.end();
    ...
    sp_debug_injector("LOADED - Library %s at %lx", sp_filename((*i)->getName().c_str()),
             loaded_addr);
  }
...
bool SpInjector::IsLibraryLoaded(const char *libname) {
  ...
  sp_debug_injector("CHECKING - checking whether %s is loaded ...", name_only);
  ...
  if (strstr(content, name_only)) {
    sp_debug_injector("LOADED - %s is already loaded", name_only);
    return true;
  }
  sp_debug_injector("NO LOADED - %s is not yet loaded", name_only);
  return false;
}
...
bool SpInjector::Inject(const char *lib_name) {
  UpdateFrameInfo();
  if (ProcessHasLibrary(proc_->getPid(), "libmyagent")) {
    sp_print("Process %d already has library %s already", proc_->getPid(),
             lib_name);
...
void SpInjector::LoadUserLibrary() {
  sp_debug_injector("PAUSED - Process %d is paused by injector.", pid_);
  if (!proc_->stopProc()) {
  ...
  if (ij_agent_addr > 0) {
    sp_debug_injector("FOUND - Address of ij_agent function at %lx", ij_agent_addr);
  } else {
  ...
  sp_debug_injector("ALLOCATED - Buffer for load-library code in mutatee's heap"
           " at %lx of %lu bytes",
           code_addr, (unsigned long)size);
  IRPC::ptr irpc = IRPC::createIRPC(code, size, code_addr);
  irpc->setStartOffset(2);
  sp_debug_injector("POSTING - IRPC is on the way ...");
...
void SpInjector::LoadIjagent(const char *lib_name) {
  ...
  if (do_dlopen_addr > 0) {
    ...
    sp_debug_injector("STORED - Library name \"%s\" in mutatee's heap at %lx",
    ...
    sp_debug_injector("STORED - do_dlopen's argument in mutatee's heap at %lx",
             args_addr);
  ...
  // Cannot do_dlopen, then we try __libc_dl_open_mode
    ...
    sp_debug_injector("Injector [pid = %5d] - Failed to find do_dlopen, "
             "try __libc_dlopen_mode",
             getpid());
    Address libc_dlopen_addr = FindFunction("__libc_dlopen_mode");
    if (libc_dlopen_addr > 0) {
      sp_debug_injector("FOUND - Address of __libc_dlopen_mode function at %lx",
               libc_dlopen_addr);
      ...
      lib_name_addr = proc_->mallocMemory(lib_name_len);
      proc_->writeMemory(lib_name_addr, (void *)libname, lib_name_len);
      sp_debug_injector("STORED - 1st argument libname \"%s\" in mutatee's heap at %lx",
               sp_filename(libname), lib_name_addr);
...
bool SpInjector::GetResolvedLibPath(const std::string &filename,
                                    StringSet &paths) {
  ...
  // REMOVED: This chunk of code is temporarily removed, the functionality is
  // no long needed
  //
...
void SpInjector::UpdateFrameInfo() {
  sp_debug_injector("Shared memory key: %ui", IJMSG_ID+proc_->getPid());
  IjMsg *shm = (IjMsg *)GetSharedMemory(IJMSG_ID+proc_->getPid(), sizeof(IjMsg));
  shm->pc = pc();
  shm->sp = sp();
  shm->bp = bp();
  sp_debug_injector("Updated frame info pc [%ld] sp [%ld] bp [%ld]", shm->pc, shm->sp,
           shm->bp);
}
