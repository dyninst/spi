...
namespace sp {
  ...
  SpringboardWorker::run(SpPoint* pt) {
    sp_debug("worker", "SPRINGBOARD WORKER - runs");
    return install(pt);
  }
  ...
  SpringboardWorker::save(SpPoint* pt) {
    assert(pt);
		...
		sp_debug("worker", "SPRING WORKER - saves");
		bool ret = false;

		assert(pt->snip());
		ph::PatchBlock* springblk = pt->snip()->FindSpringboard();
    if (!springblk) {
      sp_debug("worker", "NO SPRING BOARD - cannot find suitable spring board");
      return false;
    }
  ...
  SpringboardWorker::install(SpPoint* pt) {
    ...
    sp_debug("worker", "%s", g_parser->DumpInsns((void*)callblk->start(),
                                     callblk->size()).c_str());
    sp_debug("worker", "}");
    ...
    if (!springblk) {
			sp_debug("worker", "FAILED TO GET A SPRINGBOARD BLOCK");
			return false;
		}

    sp_debug("worker", "BEFORE INSTALL SPRING BLK (%lu bytes) for point %lx - {",
             springblk->size(), callblk->last());
    sp_debug("worker", "%s", g_parser->DumpInsns((void*)springblk->start(),
    ...
    if (!spring || (long)spring < getpagesize()) {
			sp_debug("worker", "FAILED TO RELOCATE SPRINGBOARD BLOCK");
			return false;
		}
    ...
    if (g_as->SetMemoryPermission((dt::Address)addr,
                                  springblk->size(), perm)) {
    ...
    sp_debug("worker", "AFTER INSTALL CALL BLK (%lu bytes) for point %lx - {",
             callblk->size(), callblk->last());
    sp_debug("worker", "%s", g_parser->DumpInsns((void*)callblk->start(),
                                     callblk->size()).c_str());
    sp_debug("worker", "}");

    sp_debug("worker", "AFTER INSTALL SPRING BLK (%lu bytes) for point %lx - {",
             springblk->size(), callblk->last());
    sp_debug("worker", "%s", g_parser->DumpInsns((void*)springblk->start(),
                                     springblk->size()).c_str());
    sp_debug("worker", "}");

    sp_debug("worker", "USE SPRING - piont %lx is instrumented using 1-hop spring",
             callblk->last());
